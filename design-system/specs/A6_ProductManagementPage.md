...
- [ ] 상품 목록 다운로드 (`handleDownloadExcel`): 양식 v2와 **동일 열 이름·순서(A~T 20열, 괄호 풀네임)** — 다운로드→수정→업로드 라운드트립(§엑셀 컬럼 매핑 참조).
...
- **목록 다운로드**(`handleDownloadExcel`): 양식 v2와 **동일 열 이름·순서(A~T 20열, 괄호 풀네임 헤더)** → 다운로드→수정→업로드 라운드트립. 값 포맷=파서 규약(온라인코드=포함/미포함/공란, 공용=Y/공란, 판매여부·할인여부·배지=Y/N). `test_groups` 조인으로 위계 열 채움. 1행 헤더(안내문 없음)여도 파서 헤더 탐지가 흡수.
