...
> 마지막 갱신: 2026-06-29 **소분류·배지 마스터 CRUD를 상품 관리(A6)로 이동 — 본 화면에서 블록 4·5 제거**(건우님 확정 #1). SettingsPage에는 "소분류·배지는 상품 관리 화면에서 관리합니다" 안내 Alert 1줄만 잔존. 마스터 단일 진실 소스 UI는 이제 A6.
...
- **(2026-06-29 이동) 소분류·배지 마스터 CRUD는 A6 상품 관리 화면 소관** — 건우님 확정으로 SettingsPage→ProductManagementPage 헤더 액션부로 이동. 마스터 단일 진실 소스 UI·데이터 모델(`subcategories`/`badges`)·다이얼로그·삭제 가드·색 프리셋은 이제 `A6_ProductManagementPage.md` 참조. 본 화면에는 안내 1줄만 잔존. API `src/api/masters.js`.
...
### 블록 4·5 — 소분류·배지 관리 → **A6 상품 관리로 이동 (2026-06-29 건우님 확정)**
> 두 마스터 CRUD 블록은 SettingsPage에서 **제거**되어 ProductManagementPage 헤더 액션부 "소분류·배지 관리" 토글 패널로 옮겨졌다. 상세 사양(목록·다이얼로그·삭제 가드·색 프리셋·즉시 저장 흐름)은 `A6_ProductManagementPage.md` §소분류·배지 마스터 관리 패널 참조.
- [ ] SettingsPage에는 **안내 Alert 1줄**만 잔존: "소분류·배지는 상품 관리 화면(상단 '소분류·배지 관리')에서 관리합니다." (severity info, 시스템 정보 블록 아래·하단 안내 Alert 위)
...
### (P1) 소분류·배지 마스터 CRUD — **A6 상품 관리로 이동 (2026-06-29)**
> 마스터 CRUD 액션(fetch·추가·편집·삭제·즉시 저장·색 프리셋)은 본 화면에서 제거됨. 상세는 `A6_ProductManagementPage.md` 참조. 즉시 저장 흐름·삭제 가드·프리셋 규칙은 A6에서 동일하게 유지된다.
...
- [ ] **(P1) 소분류·배지 추가/편집 다이얼로그 → A6로 이동.** 입력 폼 구조(소분류 4필드·배지 3필드 분리)는 A6 시트 참조.
...
> **(2026-06-29) 아래 `subcategories`·`badges` 마스터 테이블은 A6 상품 관리 소관으로 이동.** 데이터 모델 정의는 `A6_ProductManagementPage.md` 참조(스키마 자체는 동일, UI 소관만 이동).
...
8~11. **(P1) 마스터 CRUD 관련 핵심 발견(저장 흐름 분리·삭제 가드·색 프리셋·소분류 노출 전용)은 A6 상품 관리로 이동.** `A6_ProductManagementPage.md` §발견·§소분류·배지 마스터 관리 패널 참조. 규칙 자체는 A6에서 동일하게 유지된다.
