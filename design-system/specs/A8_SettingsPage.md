...
> 마지막 갱신: 2026-06-29 소분류·배지 마스터 CRUD **실 코드 구현** (블록 4·5). API `src/api/masters.js` 신설, SettingsPage에 SectionCard 2개·다이얼로그 2개 추가.
...
- **(계획) 소분류·배지 마스터 CRUD**: `DOCS/PRD_오티즘_카테고리배지_동적화.md` (P1 트랙). 신규 마스터 테이블 `subcategories`(name, parent_category, color, sort)·`badges`(name, color, priority). DRAFT 마이그레이션: `20260619000000_DRAFT_add_product_badges.sql` 등. **본 화면이 마스터 CRUD의 단일 진실 소스 UI** — A6 상품 폼·엑셀은 이 마스터를 소비.
...
### 블록 4 — 소분류 관리 (P1 신규 — 카테고리 동적화 PRD)
> **배치 결정:** 설정 페이지 내 신규 `SectionCard`. 학회 직전 1회 세팅하는 성격(상품 진열 준비)이라 설정 화면이 자연스럽다(알림 토글처럼 현장 즉석 조작이 아님 — §변경이력 2026-06-15 판단 기준과 동일 결). 시스템 정보 블록 아래·배송비 정책 블록 인근에 둔다. **별도 화면 신설 불필요** — SectionCard 추가가 점진 개선 원칙(CLAUDE.md 금지: 전면 재작성)에 맞음.
- [ ] 섹션 제목: "소분류 관리" (`SectionCard.title`, `CategoryIcon` 또는 `AccountTreeIcon`)
- [ ] 부제: "대분류(검사/도서/도구) 하위의 분류입니다. 고객 주문서에서 칩으로 노출되며 탐색에 쓰입니다. (매출 집계에는 영향 없음)"
- [ ] **목록(표 또는 칩 리스트):** 등록된 소분류 행 — 표시 항목:
  - 소분류명 (`subcategories.name`)
  - 소속 대분류 (`subcategories.parent_category`, 검사/도서/도구 중 하나) — 칩
  - 색 (`subcategories.color`) — 색 견본 스와치(소프트 틴트 미리보기)
  - 정렬 순서 (`subcategories.sort`) — 고객 화면 칩 노출 순서
  - 사용 상품 수 (해당 소분류명을 `products.sub_category`로 가진 상품 카운트) — 삭제 차단 판단용
  - 작업: 편집 / 삭제 아이콘
- [ ] **추가 버튼**("소분류 추가", contained `AddIcon`) → 추가 다이얼로그(이름·소속 대분류 Select·색·정렬 순서)
- [ ] 빈 상태: "등록된 소분류가 없습니다 · 추가해 시작하세요"
- [ ] **삭제 가드레일:** 사용 중(상품 1개 이상 연결)이면 삭제 차단 + "이 소분류를 쓰는 상품 {N}개가 있어 삭제할 수 없습니다." (A5 학회 삭제·A6 전체삭제와 동일한 '연결 있으면 차단' 패턴). 0개면 삭제 가능. (대안: "미등록 강등 + 경고"는 backend 정책 — 확인 필요)

### 블록 5 — 배지 관리 (P1 신규 — 카테고리 동적화 PRD)
> **배치 결정:** 소분류 관리와 동일 패턴의 SectionCard. 두 블록은 같은 마스터 CRUD 멘탈모델(추가→이름·색→사용중 삭제 차단)이라 인접 배치해 학습 비용 절감.
- [ ] 섹션 제목: "배지 관리" (`SectionCard.title`, `SellIcon` 또는 `NewReleasesIcon`)
- [ ] 부제: "상품 카드에 표시되는 강조 라벨입니다. (예: 추천, 한정) 카드당 최대 2개가 우선순위 순으로 노출됩니다."
- [ ] **목록:** 등록된 배지 행 — 표시 항목:
  - 배지명 (`badges.name`)
  - 색 (`badges.color`) — **소프트 틴트 칩 미리보기**(C1·A6에서 실제 보이는 형태로). 솔리드 칩 아님(C1 §배지 패턴 정합).
  - 우선순위 (`badges.priority`) — 숫자. 카드당 최대 2개 노출 시 정렬 기준
  - 사용 상품 수 (`products.badges`에 이 이름을 포함한 상품 카운트)
  - 작업: 편집 / 삭제 아이콘
- [ ] **추가 버튼**("배지 추가", contained `AddIcon`) → 추가 다이얼로그(이름·색·우선순위)
- [ ] 빈 상태: "등록된 배지가 없습니다 · 추가해 시작하세요"
- [ ] **삭제 가드레일:** 사용 중이면 차단 + "이 배지를 쓰는 상품 {N}개가 있어 삭제할 수 없습니다." 0개면 삭제 가능.
- [ ] **참고 안내(보조 텍스트):** "인기·신상품·할인 배지는 상품별 체크박스로 별도 관리됩니다(상품 관리 화면)." — 동적 배지와 boolean 플래그 공존을 운영자에게 명시(혼동 방지).

### 블록 6 — 액션 (현행 line 403-420)
- [ ] 우측 정렬, gap 2
...
### (P1) 소분류·배지 마스터 CRUD — 신규 액션
> 이 두 마스터는 배송비·active_event_slug와 **저장 흐름이 다르다.** 배송비는 하단 "저장하기" 일괄 저장이지만, 마스터 CRUD는 **각 행 추가/편집/삭제가 즉시 DB 반영**(별도 다이얼로그 저장 = 즉시 insert/update/delete). 하단 "저장하기"는 site_settings 전용이고 마스터에는 관여 안 함. 시안/구현에서 이 분리를 명확히(마스터 변경이 "저장하기" 안 눌러도 적용됨).
- [ ] 진입 시 `subcategories`·`badges` 마스터 + 사용 카운트 집계 fetch (각 정렬: subcategories=parent_category, sort / badges=priority)
- [ ] 소분류 추가/편집 → `subcategories` insert/update (이름·parent_category·color·sort). 같은 대분류 내 이름 중복 검증.
- [ ] 소분류 삭제 → 연결 상품 카운트 조회 → 0이면 delete, >0이면 차단 경고
- [ ] 배지 추가/편집 → `badges` insert/update (이름·color·priority). 이름 중복 검증.
- [ ] 배지 삭제 → 연결 상품 카운트 조회 → 0이면 delete, >0이면 차단 경고
- [ ] 색 입력: **프리셋 색 선택로 확정(2026-06-29 구현)**. `categoryColors.js`의 `MASTER_COLOR_PRESETS`(9색, theme 토큰 계열·AA 통과) 견본 원형 클릭. **자유 hex 미허용**(디자인 일관성·AA 대비 보장).
...
- [ ] **(P1) 소분류 추가/편집 다이얼로그:** 이름(text) + 소속 대분류(Select, 검사/도서/도구) + 색(프리셋 선택) + 정렬 순서(number) — **4개 별도 필드, 통합 금지.**
- [ ] **(P1) 배지 추가/편집 다이얼로그:** 이름(text) + 색(프리셋 선택) + 우선순위(number) — **3개 별도 필드.**
...
### `badges` 테이블 (P1 신규 — DRAFT `20260619000000`)
- `id` (PK)
- `name` (text) — 배지명. `products.badges`(text[]) 원소와 이름 자연키 연결.
- `color` (text) — 소프트 틴트 색
- `priority` (integer) — 카드당 최대 2개 노출 시 정렬 기준(낮을수록 우선 등 — backend 규약 확인 필요)
- RLS: products 동형 복제.
