...
> 마지막 갱신: 2026-06-24 카테고리·배지 동적화 PRD — 소분류·배지 마스터 CRUD 블록(P1) 추가(사양만, 실 코드 미변경).
...
- [ ] 색 입력: 디자인 토큰 팔레트에서 고르는 **프리셋 색 선택**(자유 hex 입력보다 제한 — 디자인 일관성·AA 대비 보장). 팔레트는 `categoryColors.js`/theme 토큰 계열로 한정 제안. (자유 hex 허용 여부 = 확인 필요)
