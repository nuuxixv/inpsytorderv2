...
14. **검사군 진열은 graceful이어야 한다.** `test_group_id`가 없는 환경(마이그레이션 미적용)·미분류 상품은 전부 평면(기존 그리드)으로 떨어진다. 미적용이어도 화면이 깨지면 안 됨. 검색은 **검사군 단위**(검사명·약어 1차 → 옵션명(`option_name`)·**말머리(`option_label`)** fallback, 옵션 낱개 쏟아냄 없음). **검색은 250ms 디바운스**(`useDebouncedValue`) — 입력은 즉시 반영되되 필터/검색 재계산은 타이핑이 멈춘 뒤 1회만(매 키입력 리렌더 방지). **자동 펼침은 초기 상태로만** — 검색 매칭 검사군은 처음에 펼쳐지되, 사용자가 각 카드를 자유롭게 접고 펼 수 있다(검색 중에도). 검색어 변경 시 사용자 펼침 조작은 리셋된다. 같은 검사군 여러 옵션 담기는 `order_items`에 개별 product로 자연 저장(스냅샷 4필드 계약 불변).
