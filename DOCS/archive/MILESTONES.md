...
## 🗄️ 이전 히스토리 아카이브 (Detailed Archive)

*과거 `inpsyt-order-frontend/milestones.md`에서 이관된 상세 기록입니다.*

### 2025년 7월 19일
1. `/smartadmin` 경로 리다이렉션 및 라우팅 문제 해결
2. 로그인 후 어드민 페이지 이동 문제 해결
3. `AdminHeader.jsx` `useState` 오류 해결
4. `/admin/products` 페이지 컬럼 불일치 및 UI 개선 (is_discountable 체크박스 등)
5. `/admin/events` 페이지 기능 개선 (start_date, end_date UI 개선, 실시간 구독)
6. `/admin/orders` 페이지 주문 상세 모달 개선 (주문 상태 업데이트, RLS 정책 오류 접근)

### 2025년 7월 23일
7. URL 쿼리 파라미터를 이용한 학회별 주문 관리 구현 (`?events=학회슬러그`)
8. Supabase Edge Function을 통한 안전한 주문 처리 시스템 구축 (`create-order` 함수 구현 및 트랜잭션 처리)

### 2025년 8월 1일
9. 관리자 패널 기능 개선 (주문 페이징, 엑셀 다운로드 개선)
10. 알림 시스템 개선 (실시간 알림 Snackbar, 이메일 학회명 추가)
