...
- **알림톡 승인 완료**. 현장(태블릿) 발송 실패 원인 = **원샷 IP 화이트리스트(사무실 IP만 허용)** — 발송이 브라우저 직호출이라 현장 IP가 차단됨(result_code 101). TLS 구형 제약은 서버사이드 릴레이(Deno Edge Fn)를 막은 별개 원인. 해결 진행 중(2026-06-10~): C 발송결과 기록·UI / B 원샷에 고정IP 추가등록 문의 / A Vercel 릴레이+고정IP 프록시
