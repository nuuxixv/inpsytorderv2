...
--
-- ---------------------------------------------------------------------
-- [적용 순서 — 엄수] Supabase 대시보드 → SQL Editor → (검수·승인 후) 본 파일 전문 1회.
--   ⚠ 이 마이그레이션(특히 orders.has_online_code)은 반드시
--     create-order Edge Function 배포보다 "먼저" 적용해야 한다.
--     create-order 가 insert 페이로드에 has_online_code 를 무조건 포함하므로,
--     컬럼 부재 상태로 함수만 배포되면 전 주문 생성이 실패한다(학회 중 치명적).
--     순서: ① 본 마이그레이션 → ② products 씨딩 → ③ create-order 배포.
-- ---------------------------------------------------------------------
--
