...
// 박수 = 범위(종료−시작) 그대로. 평일 필터 없음(건우님 확정).
export function tripNights(block) {
  return Math.max(0, dayDiff(block.start, block.end));
}

// 1인 수당.
export function perMemberAmount(block) {
  if (block.type === 'weekend') return weekendUnit(block) * (block.dates?.length || 0);
  return TRIP_RATE * tripNights(block);
}
...
// ─── 전체 요약(모달·export 공용) ───────────────────────────────
// 반환: { blocks: [{ ...block, slot, unit, nights, per, total }], total, paidCount, conflicts }
export function summarize(blocks) {
  const list = (blocks || []).map((b) => ({
    ...
    nights: b.type === 'weekend' ? 0 : tripNights(b),
